...
class SeniorAgent(BaseMinerNeuron):
    ...
    def __init__(
        ...
        self.llm = openai.AsyncOpenAI(base_url="https://openrouter.ai/api/v1",api_key="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")
        #

        self.model_small = "google/gemini-2.5-flash-preview"
        self.model_medium = "google/gemini-2.5-flash-preview"
        self.model_large = "meta-llama/llama-4-maverick"
    ...
    async def after_action_review(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            recent_reflections = ""
            for idx, r in enumerate(self.memory.memory["reflections"][-2:]):
                recent_reflections += f"  {idx + 1}. {r}\n"
                
            recent_action_log = ""
            for idx, l in enumerate(self.memory.memory["logs"][-15:]):
                repeat_str = (
        ...
        try:
            ...
            try:
                response = await self.llm.chat.completions.create(
                    ...
                    tool_choice="auto", 
                    timeout=8  # Shorter timeout for faster response
                )
            except Exception as e:
